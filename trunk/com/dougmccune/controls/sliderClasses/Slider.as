...
public class Slider extends UIComponent
{
    ...
    private var highlightHitArea:UIComponent;
    ...
    */
    ...
    override protected function createChildren():void
    {
        ...
        if (!highlightHitArea)
		{
			...
			innerSlider.addChild(highlightHitArea); // highlightHitArea should always be on top
   ...
   /**
	...
	private function createHighlightTrack():void
	{
		...
		if (!highlightTrack && showTrackHighlight)
		{
			...
			highlightTrack.addEventListener(MouseEvent.MOUSE_DOWN,
										   highlight_mouseDownHandler);
		}
	}
	...
	mx_internal function drawTrackHighlight():void
	{
		...
		if (highlightTrack)
		{
			...
			if (_thumbCount == 2)
			{
				var secondThumb:SliderThumb;
				
				for(i=0; i<thumbs.numChildren; i++) {
					if(firstThumb != thumbs.getChildAt(i)) {
						secondThumb = SliderThumb(thumbs.getChildAt(i));
						break;
					}
				}
			...
			highlightTrack.visible = tWidth > 0;
			
			highlightTrack.setActualSize(tWidth > 0 ? tWidth : 0, highlightSkinHeight);
			highlightTrack.move(xPos, track.y + track.height/2 - highlightSkinHeight/2);
    ...
    private function highlight_mouseDownHandler(event:MouseEvent):void
	{
		...
		var firstThumb:SliderThumb = SliderThumb(thumbs.getChildAt(0));
		event.localX = firstThumb.globalToLocal(new Point(x, y)).x;
		event.localY = firstThumb.globalToLocal(new Point(x, y)).y;
		
		firstThumb.dispatchEvent(event);
		if (_thumbCount > 1)
		{
			var secondThumb:SliderThumb = SliderThumb(thumbs.getChildAt(1));
			event.localX = secondThumb.globalToLocal(new Point(x, y)).x;
			event.localY = secondThumb.globalToLocal(new Point(x, y)).y;
		
		
			secondThumb.dispatchEvent(event);
		}
